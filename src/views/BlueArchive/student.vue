    ...
    <n-layout>
      <n-layout-header>
        ...
        <n-space v-show="searchShow" v-else class="space-main" :style="custStyle">
          ...
          <n-select :value="position" @update:value="handlePosition" :options="relationInfo?.position" :style="ua?'width: 200px':''" />
          <n-select :value="weapon" @update:value="handleWeapon" :options="relationInfo?.weapon" :style="ua?'width: 200px':''" />
          <n-input :value="searchName" @keyup.enter="handleNameChg" @update:value="handleName" type="text" placeholder="搜索学员名" :style="ua?'width: 200px':'min-width: 200px;'" />
      ...
      <n-layout content-style="overflow-x: scroll;">
        ...
        <n-space v-else :style="custStyle">
          <n-pagination
            ...
            :item-count="total"
            :on-update:page="searchStudentList"
            show-size-picker
